module.exports = function(io, targets) {
  ...
  UserSession.prototype.sendMessage = function(message) {
    try {
      logger.debug(
        'Connection ' + this.id + ' send message: ' + JSON.stringify(message)
      );
  ...
  CallMediaPipeline.prototype.setIceCandidate = function(webRtcEndpoint, user) {
        ...
        if (self.record) {
              ...
              pipeline.create(
                ...
                function(error, elements) {
                  ...
                  callerWebRtcEndpoint.connect(callerRecorder, function(error) {
                    if (error) return onError(error);
                    calleeWebRtcEndpoint.connect(calleeRecorder, function(
                      error
                    ) {
                      if (error) return onError(error);
          ...
          pipeline.create(
            ...
            function(error, elements) {
              ...
              self.setIceCandidate(callerWebRtcEndpoint, caller);
              callerWebRtcEndpoint.connect(callerWebRtcEndpoint, function(
                error
              ) {
                if (error) {
                  pipeline.release();
  ...
  function call(callerId, to, from, sdpOffer) {
    ...
    if (userRegistry.getByName(to)) {
      ...
      error = callee.sendMessage(message);
      if (error) return onError(error);
    } else {
      error = 'User ' + to + ' is not registered';
  ...
  function loopbackCallResponce(callerId, sdpOffer) {
    ...
    pipeline.createPipeline(caller, caller, function(error) {
      ...
      pipeline.generateSdpAnswer(caller.id, sdpOffer, function(
        error,
        callerSdpAnswer
      ) {
        if (error) {
  ...
  function onIceCandidate(sessionId, _candidate) {
    ...
    if (
      ...
      pipeline.webRtcEndpoint[sessionId]
    ) {
