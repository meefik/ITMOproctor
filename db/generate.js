...
var DatabaseGenerator = {
  ...
  randomizeUserRoles: function() {
    ...
    for (var i = 0; i < proportions.length; i++) {
      // set amount for each of the roles
      amountEach[i] = Math.floor(proportions[i] * this.rowAmount);
      countAmount += amountEach[i];
    }
    if (countAmount < this.rowAmount) {
      // if all amount < rowAmount -> add guests
  ...
  generateUsers: function() {
    ...
    for (var i = 0; i < this.rowAmount; i++) {
      ...
      var randomUser = {
        ...
        salt: 'secret',
        firstname:
          firstnames[this.randomizeNumber(-0.5, firstnames.length - 1)],
        lastname: lname,
        middlename: midnames[this.randomizeNumber(-0.5, midnames.length - 1)],
        email: this.transliterateString(lname) + '@example.com',
        birthday: moment(
          this.randomizeNumber(1, 28) +
            '.' +
            this.randomizeNumber(0.7, 12) +
            '.' +
            this.randomizeNumber(1980, 1990),
          'DD.MM.YYYY'
        ),
        role: userRoles[i],
        photo: [
  ...
  randomizeExamStatus: function(studentsAmount, amount) {
    ...
    for (var i = 0; i < proportions.length; i++) {
      // set amount for each of the roles
      amountEach[i] = Math.floor(
        proportions[i] * studentsAmount * this.subjectsAmountForEachStudent
      );
      countAmount += amountEach[i];
    }
    if (countAmount < studentsAmount * this.subjectsAmountForEachStudent) {
      // if all amount < rowAmount*3 -> add status_0
      while (
        countAmount !=
        studentsAmount * this.subjectsAmountForEachStudent
      ) {
        amountEach[0]++;
        countAmount++;
      }
    ...
    for (
      var i = 0;
      i < studentsAmount * this.subjectsAmountForEachStudent;
      i++
    ) {
      // randomize roles
  ...
  generateExams: function() {
    ...
    User.find(function(err, users) {
      ...
      // set exams
      var examStatus = self.randomizeExamStatus(
        students.length,
        students.length * this.subjectsAmountForEachStudent
      );
      var examCounter = 0;
      students.forEach(function(element, index, array) {
        var uId = element;
        var exams =
          self.subjectsGenerated[
            self.randomizeNumber(0, self.subjectsGenerated.length - 1)
          ];
        ...
        exams.forEach(function(element, index, array) {
          ...
          case 0: //ожидает
            ...
            eResolution = null;
            eStartDate = moment(
              self.randomizeNumber(eBeginDate.valueOf(), now.valueOf())
            );
            eStopDate = null;
            eCurator = [];
            break;
          ...
          case 2: //сдан
            var date =
                checkDay(self.randomizeNumber(1, 28)) +
                '.' +
                checkMonth(now.month() + 1 - self.randomizeNumber(1, 5)) +
                '.' +
                now.year();
            ...
            eStartDate = moment(
              self.randomizeNumber(
                eBeginDate.valueOf(),
                eBeginDate.valueOf() + 1800000
              )
            );
            eStopDate = moment(
              self.randomizeNumber(
                eStartDate.valueOf() + 3600000,
                eEndDate.valueOf()
              )
            );
            eCurator = self.setCurators(users);
          ...
          case 4: //пропущен
            var date =
                checkDay(now.date() - self.randomizeNumber(1, 10)) +
                '.' +
                checkMonth(now.month() + 1 - self.randomizeNumber(-0.1, 5)) +
                '.' +
                now.year();
            var startHour = self.randomizeNumber(6, 13);
            eBeginDate = moment(
          ...
          case 5: //запланирован
            ...
            var date =
                checkDay(self.randomizeNumber(1, 28)) +
                '.' +
                checkMonth(now.month() + 1 + self.randomizeNumber(0.7, 5)) +
                '.' +
                now.year();
            eBeginDate = moment(
              date + ' ' + startHour + ':00',
