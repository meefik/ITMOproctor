...
var db = {
  ...
  storage: {
    upload: function(files, callback) {
      ...
      files.forEach(function(file) {
        ...
        fs.exists(fullname, function(exists) {
          ...
          var uploadStream = db.gridFSBucket.openUploadStreamWithId(
            fileId,
            file.filename,
            {
    ...
    download: function(fileId, callback) {
      if (typeof fileId !== 'object') fileId = new ObjectID(fileId);
      db.gridFSBucket.find({ _id: fileId }).next(function(err, info) {
        if (err || !info) return callback(err);
        var downloadStream = db.gridFSBucket.openDownloadStream(fileId);
        downloadStream.pipe(callback(info));
      });
    },
    remove: function(files, callback) {
      ...
      files.forEach(function(file) {
        var fileId = file.fileId;
        if (typeof fileId !== 'object') fileId = new ObjectID(fileId);
        db.gridFSBucket.delete(fileId, callback);
      });
    },
  ...
  exam: {
    ...
    search: function(args, callback) {
      ...
      Exam.countDocuments(query, function(err, count) {
        if (err || !count) return callback(err);
        Exam.find(query)
          .sort('leftDate beginDate subject')
          .skip(rows * page)
          .limit(rows)
          .populate(opts)
          .exec(function(err, data) {
            ...
            Exam.update(
              ...
              },
              {
                $set: {
                  ...
                  inspector:
                    data.inspectors[Math.floor(Math.random() * amount)],
                  beginDate: beginDate,
                  endDate: endDate,
                  planDate: moment()
                }
    ...
    schedule: function(args, callback) {
      ...
      var leftDate = moment
        .max(now, moment(args.data.leftDate))
        .startOf('hour');
      ...
      Schedule.find({
        ...
        for (var i = 0, li = schedules.length; i < li; i++) {
          ...
          var times = moment
            .min(rightDate, endDate)
            .diff(beginDate, 'hours', true);
        ...
        Exam.find({
          ...
          for (var i = 0, li = exams.length; i < li; i++) {
            ...
            var times = moment
              ...
              .diff(beginDate, 'hours', true);
            for (
              var j = start < 0 ? 0 : start, lj = start + times;
              j < lj;
              j++
            ) {
              if (timetable[inspector] && timetable[inspector][j] > 0)
      ...
      var opts = [
        {
          path: 'student',
          select:
            'provider firstname lastname middlename gender birthday citizenship documentType documentNumber documentIssueDate address description'
        },
        {
          path: 'inspector',
          select: 'firstname lastname middlename'
  ...
  schedule: {
    ...
    add: function(args, callback) {
      ...
      var now = moment()
        .add(offset, 'hours')
        .startOf('hour');
      if (beginDate >= endDate || beginDate < now || args.data.concurrent < 1)
        return callback();
