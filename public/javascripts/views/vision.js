...
define([
  ...
  var View = Backbone.View.extend({
    ...
    render: function() {
      ...
      this.$ScreenshotComment = this.$DialogScreenshot.find(
        '.screenshot-comment'
      );
      ...
      this.$ProtectionCodeInput = this.$DialogConfirm.find(
    ...
    getExamStatus: function() {
      var self = this;
      $.getJSON('inspector/status/' + this.options.examId, function(data) {
        if (!data) return;
    ...
    doVerify: function() {
      var self = this;
      this.view.verify.doOpen(
        this.$Video.get(0),
        this.exam.get('_id'),
        this.exam.get('student'),
        {
          success: function(model) {
            var message = model.get('submit')
              ? i18n.t('vision.verify.success')
              : i18n.t('vision.verify.fail');
            self.view.notes.collection.create({
              time: app.now(),
              text: message,
    ...
    confirmDlg: function(resolution) {
      ...
      if (resolution) {
        this.$ConfirmMessage.html(
          i18n.t('vision.submit.message', {
            resolution:
              '<strong style="color:green">' +
              i18n.t('vision.submit.true') +
              '</strong>'
          })
        );
      } else {
