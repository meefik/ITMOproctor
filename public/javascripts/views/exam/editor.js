...
// ExamEditor view
//
define(['i18n', 'text!templates/exam/editor.html'], function(i18n, template) {
    ...
    doSave: function() {
      ...
      this.$From.serializeArray().map(function(item) {
        if (config[item.name]) {
          ...
          if (item.name == 'duration' && item.value != null)
          ...
          if (item.name == 'rightDate') {
            if (item.value)
              item.value = moment(item.value, 'DD.MM.YYYY').endOf('day');
          }
          ...
          if (item.name == 'startDate' || item.name == 'stopDate') {
            if (item.value)
              item.value = moment(item.value, 'DD.MM.YYYY HH:mm:ss');
          }
          // set config
