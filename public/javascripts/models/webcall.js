...
define([], function() {
  ...
  var Model = Backbone.Model.extend({
    ...
    incomingCall: function(message) {
      ...
      kurentoUtils.WebRtcPeer.WebRtcPeerSendrecv(this.getOptions(), function(
        error
      ) {
    ...
    call: function(peer) {
      ...
      if (this.get('input') && !this.get('output')) {
        kurentoUtils.WebRtcPeer.WebRtcPeerSendonly(this.getOptions(), function(
          error
        ) {
          if (error) return self.onError(error);
          this.generateOffer(onOffer);
      ...
      if (!this.get('input') && this.get('output')) {
        kurentoUtils.WebRtcPeer.WebRtcPeerRecvonly(this.getOptions(), function(
          error
        ) {
          if (error) return self.onError(error);
          this.generateOffer(onOffer);
    ...
    showSpinner: function() {
      for (var i = 0; i < arguments.length; i++) {
        ...
        arguments[i].style.background =
          'center transparent url("images/spinner.gif") no-repeat';
      }
    },
    ...
    toggleAudio: function(flag) {
      ...
      if (this.webRtcPeer) {
        var audioTracks = this.webRtcPeer.peerConnection
          .getLocalStreams()[0]
          .getAudioTracks();
    ...
    toggleVideo: function(flag) {
      ...
      if (this.webRtcPeer) {
        var videoTracks = this.webRtcPeer.peerConnection
          .getLocalStreams()[0]
          .getVideoTracks();
